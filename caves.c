...
map_t * simulation_step(map_t * _map, int birth_limit, int death_limit, int num_of_steps)
{
	...
	memcpy(_buffer->buffer, _map->buffer, sizeof(tile_t)*_map->height*_map->width);
	for(int k = 0; k < num_of_steps; k++)
	{
		for(int i = 1; i < _map->height-1; i++)
		{
			for(int j = 1; j < _map->width-1; j++)
			{
				...
				if         ((buffer[i][j].flags & FLAG_SOLID) && (neighbour >= death_limit))
				{
					land[i][j].symbol = '.';
					land[i][j].flags ^= FLAG_SOLID;
					land[i][j].flags ^= FLAG_TRANS;
				...
				else if((!(buffer[i][j].flags & FLAG_SOLID))  && (neighbour >= birth_limit))
				{
					land[i][j].symbol = '#';
					land[i][j].flags |= FLAG_SOLID;
					land[i][j].flags |= FLAG_TRANS;
