...
void create_line (int x1, int y1, int x2, int y2, char ch, int COLOR, map_t* map) {
	...
	if (x1-x2 == 0) {
		...
		while (y != y2) {
			...
			field[(int)y][(int)x].flags |= FLAG_SOLID;
			if (y2 >= y) {
				x++;
				y++;
			} else {
...
void div_room (tree_room_t* parent, vector_room_t* leaves) {
	room_tree_t* parent_room = parent->value;
	if(parent_room->S <= 64 || (parent_room->len_x <=16 && parent_room->len_y <= 8)) {
		vector_room_add(leaves, parent);
		return;
	}
	room_tree_t* next_rooms[2];
	next_rooms[0] = malloc(sizeof(room_tree_t));
	next_rooms[1] = malloc(sizeof(room_tree_t));
	int len_last;
	...
	div_room(parent->right, leaves);
	div_room(parent->left, leaves);
}
...
void div_walls_of_rooms (tree_room_t*	leaves, map_t* map) {
	...
	if (leaves->parent != NULL && leaves != NULL) {
		room_tree_t* room = leaves->value;
		room_tree_t* parent_room;
		...
		if (parent_room->x1 > room->x1) {
			...
			while (field[room->door_y[0]][room->door_x[0]+1].symbol  == FLAG_SOLID ||
				field[room->door_y[0]][room->door_x[0]-1].symbol ==  FLAG_SOLID ||
				field[room->door_y[1]][room->door_x[1]+1].symbol  ==  FLAG_SOLID ||
				field[room->door_y[1]][room->door_x[1]-1].symbol == FLAG_SOLID ) {
				room->door_y[0]++;
				room->door_y[1]++;			
			}
			...
			while (field[room->door_y[0]+1][room->door_x[0]].symbol  ==  FLAG_SOLID ||
				field[room->door_y[0]-1][room->door_x[0]].symbol  ==  FLAG_SOLID ||
				field[room->door_y[1]+1][room->door_x[1]].symbol  ==  FLAG_SOLID ||
				field[room->door_y[1]-1][room->door_x[1]].symbol  == FLAG_SOLID ) {
				room->door_x[0]++;
				room->door_x[1]++;
			}
		...
		for(int i = 0; i<2; i++) {
			...
			field[room->door_y[i]][room->door_x[i]].flags &= ~FLAG_SOLID;
		}
	} else {
		return;
	}
