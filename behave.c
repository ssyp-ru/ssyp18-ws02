...
int behave_fire(actor_t* self){
	...
	bullet->id = give_actor_id();
	bullet->targ_x = 0;
	bullet->targ_y = 0;
	bullet->flags = 0;
	bullet->flags |= FLAG_ACTOR_PROJECTILE;
...
int behave_projectiles(actor_t* self){
	...
	if (self->level->actors != NULL){
		for (int i = 0; i < self->level->actors->length; i++){
			...
			if (self->level->actors->data[i]->x == self->x
				...
				&& !(self->level->actors->data[i]->flags & FLAG_ACTOR_PROJECTILE)){
			//------------------------------------------------------
				self->level->actors->data[i]->hp -= 3;

				if (self->level->actors->data[i]->hp <= 0){
					self->level->actors->data[i]->flags |= FLAG_ACTOR_DEAD;
				}
	...
	if (map[self->y][self->x].flags & FLAG_TILE_SOLID){
		self->flags |= FLAG_ACTOR_DEAD;
	}
	return 1;
}
...
int behave_searchtarget(actor_t* self){
	...
	if (flag != 1 && (self->flags & FLAG_ACTOR_CANWALK)){
		int i = rand() % 4;
		switch(i){
			case 0:
				if (self->x + 1 >= self->level->map->width)
					break;
				if(map[self->y][self->x + 1].flags & FLAG_TILE_SOLID)
					break;
				self->x += 1;
				self->direct = 0;
			break;
			...
			case 2:
				if (self->y + 1 >= self->level->map->height)
					break;
				if (map[self->y + 1][self->x].flags & FLAG_TILE_SOLID)
					break;
				self->y += 1;
				self->direct = 2;
			break;
...
int behave_chasetarget(actor_t* self){
	...
	if (self->x != self->targ_x){
		if (self->x < self->targ_x){
			if (self->x + 1 < self->level->map->width
					&& !(map[self->y][self->x + 1].flags & FLAG_TILE_SOLID)){
				self->x += 1;
				self->direct = 0;
			}
		} else {
	...
	if (self->y != self->targ_y){
		if (self->y < self->targ_y){
			if (self->y + 1 < self->level->map->height
					&& !(map[self->y + 1][self->x].flags & FLAG_TILE_SOLID)){
				self->y += 1;
				self->direct = 2;
			}
		} else {
	...
	if (self->level->actors){
		for (int i = 0; i < self->level->actors->length; i++){
			...
			//------------------------------------------------------
				...
				self->targ_y = self->level->actors->data[i]->y;
			//------------------------------------------------------
				if (self->flags & FLAG_ACTOR_MELEE){
					self->state = 2;
				} else {
					self->state = 3;
				}
