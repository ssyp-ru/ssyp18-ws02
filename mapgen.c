...
void buildWall(map_t* mapPacked, int width, int height, bool isHorizontal, int x, int y)
{
	...
	if(isHorizontal)
	{
		...
		{
			if(rand() % 15 < 13 || width > mapPacked->width / 4 || wallLoc - y > mapPacked->height / 4)
				buildWall(mapPacked, width, wallLoc - y, wallLoc - y >= width, x, y);	
			else if(width > mapPacked->width / 20 || wallLoc - y > mapPacked->height / 20)
		...
		{
			if(rand() % 15 < 13 || width > mapPacked->width / 4 || height - wallLoc + y - 1 > mapPacked->height / 4)
				buildWall(mapPacked, width, height - wallLoc + y - 1, height - wallLoc + y - 1 >= width, x, wallLoc + 1);
			else if(width > mapPacked->width / 20 || height - wallLoc + y - 1 > mapPacked->height / 20)
		...
		for(int i = x; i < x + width; i++)
		{
			if((map[wallLoc - 1][i] == '.' || map[wallLoc - 1][i] == ' ') && (map[wallLoc + 1][i] == '.' || map[wallLoc + 1][i] == ' '))
			{
	...
	else
	{
		...
		{
			if(rand() % 15 < 13 || wallLoc - x > mapPacked->width / 4 || height > mapPacked->height / 4)
				buildWall(mapPacked, wallLoc - x, height, height >= wallLoc - x, x, y);
			else if(wallLoc - x > mapPacked->width / 20 || height > mapPacked->height / 20)
		...
		{
			if(rand() % 15 < 13 || width - wallLoc + x - 1 > mapPacked->width / 4 || height > mapPacked->height / 4)
				buildWall(mapPacked, width - wallLoc + x - 1, height, height >= width - wallLoc + x - 1, wallLoc + 1, y);
			else if(width - wallLoc + x - 1 > mapPacked->width / 20 || height > mapPacked->height / 20)
		...
		for(int i = y; i < y + height; i++)
		{
			if((map[i][wallLoc - 1] == '.' || map[i][wallLoc - 1] == ' ') && (map[i][wallLoc + 1] == '.' || map[i][wallLoc + 1] == ' '))
			{
				possibleHoleSpots[it] = i;
				it++;
			}
