...
typedef struct actor{
	...
	unsigned int x, y;
	inv_t* inventory;
	unsigned int state;	//current state.For example monster search monster
	int flags;				//features of actor(maybe check actor is player)
	unsigned int targx, targy;	//-|____position of target of this actor(player, item, other actor)
	struct level  *level;
