...
void generateMap(map_t* mapp, int num){
	...
	while(z < num){
		...
		int x1 = countx * cellheight + rand() % (cellheight - (x - countx * cellheight));
		int y1 = county * cellwidth + rand() % (cellwidth - (y - county * cellwidth));
		if (x > x1){
			int swap = x;
			x = x1;
			x1 = swap;
