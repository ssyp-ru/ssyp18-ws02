...
map_t * mapgen_shrew(map_t *_map) {
	...
	for(int k = 0; k < size_y; k++) {
		for(int i = 0; i < size_x; i++) {
			map[k][i].flags |= FLAG_TILE_SOLID;
		}
	}
	origin_x = rand() % (size_x - 3) + 1;
	origin_y = rand() % (size_y - 3) + 1;
	map[origin_y][origin_x].flags &= ~FLAG_TILE_SOLID;
	while(index < count_cells) {
		...
		if(map[origin_y][origin_x].flags & FLAG_TILE_SOLID
		        && counter < 4) {
			map[origin_y][origin_x].flags &= ~FLAG_TILE_SOLID;
			index++;
		} else {
...
map_t * mapgen_test_room_shrew(map_t *_map, int y_pos,
	...
	for(int i1 = 0; i1 < 10; i1++) {
		for(int k1 = 0; k1 < 10; k1++) {
			map[i1 - 5 + y_pos][k1 - 5 + x_pos].flags &= ~FLAG_TILE_SOLID;
		...
		for(int k = rand_y; k <= rand_y + siz1; k++) {
			for(int l = rand_x; l <= rand_x + siz2; l++) {
				if(k == rand_y || k == rand_y + siz1
					...
					if(!(map[k - 1][l].flags & FLAG_TILE_SOLID) &&
					        !(map[k + 1][l].flags & FLAG_TILE_SOLID) && counter < 3) {
						//Place for a door
						map[k][l].flags &= ~FLAG_TILE_SOLID;
						counter++;
					} else if(!(map[k][l - 1].flags & FLAG_TILE_SOLID) &&
					          !(map[k][l + 1].flags & FLAG_TILE_SOLID) && counter < 3) {
						//Place for a door
						map[k][l].flags &= ~FLAG_TILE_SOLID;
						counter++;
					} else
						map[k][l].flags |= FLAG_TILE_SOLID;
				} else map[k][l].flags &= ~FLAG_TILE_SOLID;
			}
		}
		counter = 0;
	}
...
map_t * make_walls_shrew(map_t * _map) {
  ...
  for(int k = 1; k < size_y - 1; k++) {
    for(int i = 1; i < size_x - 1; i++) {
			if(map[k][i].flags & FLAG_TILE_SOLID) {
				if(k == 1 || k == size_y - 2 ||
				        i == 1 || i == size_x - 2) {
					map[k][i].symbol = '#' | COLOR_PAIR(1);
				} else {
					if(!(map[k - 1][i].flags & FLAG_TILE_SOLID) &&
